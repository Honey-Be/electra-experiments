...
class OpenWebTextArgs:
    ...
    distributed_world_size: arg.Int = 1
...
def train(rank, args, variant: type[ElectraWrapper] = ElectraDefault):
    ...
    if variant == ElectraBackbonedWithFNet:
        model_generator = FNetConfig.from_pretrained(args.model_generator)
        model_discriminator = FNetConfig.from_pretrained(args.model_discriminator)
    else:
